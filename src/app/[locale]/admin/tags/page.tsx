...
import { adminCreateTag, adminUpdateTag, listTags, TagItem } from '../../../../lib/api';
...
export default function AdminTagsPage() {
  ...
  const [updateForm, setUpdateForm] = useState<TagFormState>(EMPTY_FORM);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) =>
      [item.en_name, item.mm_name, item.slug, item.description ?? '']
        .join(' ')
        .toLowerCase()
        .includes(keyword)
    );
  ...
  function openDetail(item: TagItem) {
    setSelected(item);
    setDetailOpen(true);
  }

  function openUpdate(item: TagItem) {
    setSelected(item);
    setUpdateForm({
  ...
  async function onCreate(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const created = await adminCreateTag(
        {
  ...
  async function onUpdate(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const updated = await adminUpdateTag(
        selected.id,
        {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Sustainability Tags</h1>
          <p className="text-sm text-slate-500">Create, update and inspect catalog tags.</p>
        </div>
        <div className="flex gap-2">
          <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
            {filtered.length} tag(s)
          </span>
          <button
      ...
      <div className="mt-4">
        <input
          ...
          placeholder="Search tag by en/mm name or slug"
          value={search}
      ...
      <AdminModal open={createOpen} title="Create Tag" onClose={() => setCreateOpen(false)}>
        <form className="grid gap-3 md:grid-cols-2" onSubmit={onCreate}>
          <input
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm"
            placeholder="Tag EN name"
            required
      ...
      <AdminModal open={detailOpen} title="Tag Detail" onClose={() => setDetailOpen(false)}>
        {selected ? (
      ...
      <AdminModal open={updateOpen} title="Update Tag" onClose={() => setUpdateOpen(false)}>
